...
export default async function SearchBar_Location({}: Props) {
  ...
  return (
    <div className="absolute top-6 right-0 left-0 z-10 mx-auto flex h-[75px] w-9/12 flex-row items-center justify-between rounded-3xl bg-white px-2 py-2 drop-shadow-xl">
      ...
      <div className="flex h-full w-16 cursor-pointer items-center justify-center rounded-3xl bg-slate-800 transition-all duration-300 active:scale-95">
        <MagnifyingGlassIcon className="h-6 w-6 text-stone-50"></MagnifyingGlassIcon>
      </div>
    </div>
  );
}
